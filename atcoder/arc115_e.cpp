...
inline void pushdown(int p) {
    val[ls] = (val[ls]*tagm[p]%mod+taga[p]%mod*base[ls]%mod)%mod;
    val[rs] = (val[rs]*tagm[p]%mod+taga[p]%mod*base[rs]%mod)%mod;
...
inline void add(int p,int s,int e,int v,int l,int r) {
    if (s <= l && r <= e) {
        val[p] = (base[p]*v%mod%mod+val[p])%mod;
        taga[p] = (taga[p]+v)%mod;
        return;
    }
    int mid = (l + r) >> 1;
    if ((taga[p] || tagm[p] != 1) && l != r) pushdown(p);
